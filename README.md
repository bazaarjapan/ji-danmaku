...
- OpenAI API キー: GPT Realtime Whisper を使う場合
- Anthropic API キー: Claude Vision をブレインにする場合
...
`ANTHROPIC_API_KEY` を設定すると Claude Vision を利用できます。画面反応を行う場合、スクリーンショットや発話テキストが Anthropic API に送信されます。
...
手動で `openai` に切り替えた場合に使います。OpenAI API キーが必要です。発話音声を OpenAI Realtime API に送信するため、API 利用料金が発生します。コントロール画面には概算コストを表示しますが、正確な請求額は OpenAI 側のダッシュボードで確認してください。
...
- GPT Realtime Whisper を使う場合は、音声そのものが OpenAI に送信されます。
- Anthropic ブレインを使う場合は、画面情報や発話テキストが Anthropic API に送信されます。
...
OpenAI API キーによる従量課金が発生するのは、主に GPT Realtime Whisper を手動で有効にした場合です。Anthropic ブレインを使う場合は Anthropic API の課金条件に従います。
...
electron/
  ...
  ai/
    ...
    anthropic.js       Anthropic API 連携
    mock.js            AI 不使用の弾幕生成
    openai-stt.js      GPT Realtime Whisper 連携
...
依存ライブラリ、Electron / Chromium、Whisper モデル、Codex CLI、OpenAI API、Anthropic API には、それぞれのライセンス、利用規約、料金条件が適用されます。詳細は `THIRD_PARTY_NOTICES.md` と各サービスの公式情報を確認してください。
