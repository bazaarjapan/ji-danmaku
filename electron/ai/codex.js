...
function buildPrompt({ count, context, transcript }) {
  const ctxLine = context && (context.title || context.process)
    ...
    : '前面アプリ情報なし';
  const voiceLine = transcript ? `配信者の発話(マイク): 「${transcript}」` : '発話なし';
  return [
    'あなたはニコニコ動画/ライブ配信の視聴者です。',
    '添付のスクリーンショットは配信者(PCの持ち主)の画面です。これを見て、',
    'リアルタイムに流れる短い視聴者コメント(弾幕)を生成してください。',
    '',
    '制約:',
    `- ${count}個ちょうど生成する`,
    '- 各コメントは日本語で最大20文字程度、短く',
    '- ネットスラング(w, 草, 888, kawaii, それな 等)や実況的な相づちを混ぜる',
    '- 画面の内容に具体的に反応するコメントを半分、汎用リアクションを半分',
    '- 重複しすぎない。煽り/誹謗中傷/不適切表現は避け、明るく楽しい雰囲気',
    '- たまに color(例 "#ff5b5b") や big:true を付けて盛り上げる(全体の2割以内)',
    '',
    ctxLine,
    voiceLine,
    '',
